...
CapMemCellOnCapMemBlock const CapMemCellOnCapMemBlock::stp_i_ramp{
    CapMemColumnOnCapMemBlock(128), CapMemRowOnCapMemBlock(13)};
CapMemCellOnCapMemBlock const CapMemCellOnCapMemBlock::stp_i_calib{
    CapMemColumnOnCapMemBlock(128), CapMemRowOnCapMemBlock(14)};
...
CapMemCellOnDLS UnusedCapMemCellOnDLS::toCapMemCellOnDLS() const
{
	...
	static const std::array<CapMemCellOnDLS, size> cells = []() {
		...
		for (size_t b = 1; b < 4; ++b) {
			for (size_t r = 0; r < 23; ++r) {
				cs.at(i) = CapMemCellOnDLS(
				    CapMemCellOnCapMemBlock(
				        CapMemRowOnCapMemBlock(r), CapMemColumnOnCapMemBlock(129)),
				    CapMemBlockOnDLS(b));
				i++;
			}
		}
